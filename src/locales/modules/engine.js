...
export default {
  ...
  detectHasWarnings: _('检测到环境提示项，请确认后继续。缺失的 uv / Python 运行时会在安装时自动补齐，Git 缺失可能导致安装失败。', 'Environment notices detected — review them, then continue. Missing uv / Python are fetched automatically during install; a missing Git may cause install failures.', '檢測到環境提示項，請確認後繼續。缺失的 uv / Python 執行環境會在安裝時自動補齊，Git 缺失可能導致安裝失敗。'),
