...
export default {
  ...
  portableBadge: _('便携模式', 'Portable Mode', '便攜模式', 'ポータブルモード', '휴대용 모드', 'Chế độ di động', 'Modo portátil', 'Modo portátil', 'Портативный режим', 'Mode portable', 'Portabler Modus'),
  portableWarnNode: _('U 盘 Node 运行时不完整（runtimes/node 缺少可执行文件），将回退使用本机 Node', 'Portable Node runtime is incomplete (runtimes/node has no executable), falling back to local Node', 'USB Node 執行環境不完整（runtimes/node 缺少可執行檔），將回退使用本機 Node'),
  portableWarnCli: _('U 盘 OpenClaw 引擎不完整（engines/openclaw 缺少 CLI 入口），将回退使用本机安装', 'Portable OpenClaw engine is incomplete (engines/openclaw has no CLI entry), falling back to local install', 'USB OpenClaw 引擎不完整（engines/openclaw 缺少 CLI 入口），將回退使用本機安裝'),
  portableWarnHermesCli: _('U 盘 Hermes 引擎不完整（engines/hermes 缺少 CLI 入口），将回退使用本机安装', 'Portable Hermes engine is incomplete (engines/hermes has no CLI entry), falling back to local install', 'USB Hermes 引擎不完整（engines/hermes 缺少 CLI 入口），將回退使用本機安裝'),
  portableWarnAbsPath: _('portable.json 中的 {field} 是绝对路径，更换盘符后可能失效', '{field} in portable.json is an absolute path and may break when the drive letter changes', 'portable.json 中的 {field} 是絕對路徑，更換磁碟代號后可能失效'),
  ...
  macNodeUpgradeHint: _('macOS 请通过官网、Homebrew、nvm 或 fnm 升级到满足当前 OpenClaw 要求的 Node.js。升级后建议重启 ClawPanel；如果从 Finder 启动仍检测到旧版本，可从终端重新打开：', 'On macOS, upgrade Node.js to a version that satisfies the current OpenClaw requirement via the official installer, Homebrew, nvm, or fnm. Restart ClawPanel after upgrading; if Finder still detects the old version, reopen it from Terminal:', 'macOS 請透過官網、Homebrew、nvm 或 fnm 升級到滿足目前 OpenClaw 要求的 Node.js。升級後建議重啟 ClawPanel；如果從 Finder 啟動仍檢測到舊版本，可從終端重新開啟：'),
